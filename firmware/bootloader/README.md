...
- Позволяет стирать и прошивать пользовательскую Flash (`0x0800_8000` — `0x0803_FFFF`); application integrity metadata находится в `0x0803_D000`.
...
- приложение должно записать `0xBEEF` в backup-регистр `BKP->DR1` (переживает system reset, очищается при снятии питания) и вызвать `NVIC_SystemReset()`; для совместимости с уже прошитыми bootloader'ами приложение дополнительно пишет legacy-магию `0xDEADBEEF` по адресу `0x2000_4FF0`;
